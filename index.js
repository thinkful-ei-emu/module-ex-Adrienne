...
function renderShoppingList() {
  ...
  $('.js-shopping-list-search').val(STORE.searchText);

  if (STORE.searchText) {
    filteredItems = filteredItems.filter(item => item.name.includes(STORE.searchText));
  }
...
// type in search term and list will filter to items with term
function searchItemList(searchText) {
  STORE.searchText = searchText;
}

function handleSearchItemList() {
  // console.log('`searchItemList` ran');
  $('#js-search-list-form').on('submit', event => {
    event.preventDefault();
...
function editListItem(id, newValue) {
  console.log(`Editing '${id} in shopping list`);
  const itemIndex = STORE.items.findIndex(item => item.id === id);
  STORE.items[itemIndex].name = newValue;
}

function handleEditListItem() {
  console.log('`handleEditListItem` ran');
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    const itemId = getItemIdFromElement(event.currentTarget);
    const newValue = prompt("What's the new name?");
    console.log(newValue);
    editListItem(itemId, newValue);
    renderShoppingList();
  })
}
